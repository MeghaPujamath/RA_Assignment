...
def index(request):
    while(len(free_men) > 0):
        for man in free_men:
            begin_matching(man)
    context = {'data':tentative_engagements}
    return render(request, 'stablematching/stablematching/templates/index.html',context)
...
def begin_matching(man):
	...
	for woman in preferred_rankings_men[man]:
		...
		if (len(taken_match) == 0):
			...
			free_men.remove(man)
			print('%s is no longer a free man and is now tentatively engaged to %s' %
			      (man, woman))
			break
